    ...
    <header className="fixed inset-x-0 top-3 z-40 px-3 sm:px-5">
      <div className="mx-auto flex h-16 max-w-[1320px] items-center justify-between gap-4 rounded-full border border-[#F5F1E8]/12 bg-[#052822]/88 px-3 shadow-[0_8px_30px_rgba(0,0,0,.16)] backdrop-blur-xl sm:px-4 lg:gap-6 lg:px-5">
        ...
        <div className="hidden items-center gap-2 sm:flex">
          ...
          {isLoggedIn && currentUser && (
            <div className="relative">
              <button id="user-profile-menu-btn" type="button" onClick={() => setAuthDropdownOpen((open) => !open)} aria-expanded={authDropdownOpen} className="flex h-10 items-center gap-2 rounded-full border border-[#F5F1E8]/15 px-2 text-[11px] text-[#F5F1E8]">
        ...
        <div className="flex items-center gap-2 lg:hidden">
          <button type="button" onClick={startBooking} className="rr-button-gold h-11 px-5 text-[12px] font-black uppercase">Book</button>
          <button type="button" onClick={() => setMobileMenuOpen((open) => !open)} aria-expanded={mobileMenuOpen} aria-controls="mobile-navigation-drawer" aria-label="Toggle navigation menu" className="grid h-11 w-11 place-items-center rounded-full border border-[#F5F1E8]/20 text-[#F5F1E8]">{mobileMenuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
        </div>
      </div>

      {mobileMenuOpen && (
        <div id="mobile-navigation-drawer" className="mx-auto mt-2 max-w-[1320px] rounded-[24px] border border-[#D6A62E]/25 bg-[#073B32]/96 px-5 py-5 shadow-2xl backdrop-blur-xl lg:hidden">
